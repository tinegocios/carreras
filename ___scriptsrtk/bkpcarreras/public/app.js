...
const lastPositions = new Map(); // id -> { lat, lon }
const blinkPending = new Set();

const DEFAULT_TRACK = [
  { lat: 20.029954, lon: -98.785524 },
  { lat: 20.0304, lon: -98.785466 },
];
...
function initMap() {
  map = L.map('map').setView([19.5220529,-99.0234761], 16);
...
function setTrack(points, fit = true) {
  ...
  normalized.forEach((p, idx) => {
    ...
    const marker = makeDraggableMarker(p.lat, p.lon, idx).addTo(map);
    pointMarkers.push(marker);
  });
  redrawTrack(fit);
  return true;
...
function clearTrack() {
  ...
  pointMarkers.forEach(m => { if (map && map.hasLayer(m)) map.removeLayer(m); });
  pointMarkers = [];
  lastPositions.clear();
  blinkPending.clear();
}
...
function redrawTrack(fit=true) {
  ...
  if (track.length >= 1) {
    const first = track[0];
    if (!startMarker) {
      startMarker = L.circleMarker([first.lat, first.lon], { radius: 6, color: '#2ecc71', weight: 2, fillColor: '#2ecc71', fillOpacity: 0.7 }).addTo(map);
    } else {
      startMarker.setLatLng([first.lat, first.lon]);
    }
  } else if (startMarker) {
  ...
  if (track.length >= 2) {
    ...
    if (!finishMarker) {
      finishMarker = L.circleMarker([last.lat, last.lon], { radius: 6, color: '#e74c3c', weight: 2, fillColor: '#e74c3c', fillOpacity: 0.7 }).addTo(map);
    } else {
      finishMarker.setLatLng([last.lat, last.lon]);
    }
...
function onWSMessage(ev) {
  ...
  if (msg.type === 'tick') {
    ...
    for (const c of snap.competitors) {
      ...
      const moved = !prev || Math.abs(prev.lat - c.lat) > 1e-9 || Math.abs(prev.lon - c.lon) > 1e-9;
      if (moved) {
        blinkPending.add(key);
        lastPositions.set(key, { lat: c.lat, lon: c.lon });
      }
    }
    updateBoard(snap);
    updateChartDatasets();
...
function onLoadFile(ev) {
  ...
  reader.onload = () => {
    try {
      ...
      if (!setTrack(data)) throw new Error('Formato inválido');
    } catch (e) {
...
function updateBoard(snap) {
  ...
  for (const c of arr) {
    ...
    if (!li) {
      ...
      dot.addEventListener('animationend', () => dot.classList.remove('blink'));
    ...
    const dot = li.querySelector('.color-dot');
    if (dot && blinkPending.has(c.id)) {
      dot.classList.remove('blink');
      void dot.offsetWidth;
      dot.classList.add('blink');
      blinkPending.delete(c.id);
    }
    list.appendChild(li);
    pos++;
  }
